...
langs_by_state <- langs_by_state %>% 
  ...
  mutate(across(where(is.numeric), ~ round(., digits = 4))) %>%  #Round percentages to two decimal places
...
lang_by_state_age_group_english_level <- lang_by_state_age_group_english_level %>% 
  ...
  mutate(percent_of_speakers_in_state = total_speakers/total_speakers_in_usa*100)
